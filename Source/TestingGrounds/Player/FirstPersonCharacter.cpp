...
AFirstPersonCharacter::AFirstPersonCharacter()
{
	...
	// Default offset from the character location for BallProjectiles to spawn
	GunOffset = FVector(100.0f, 0.0f, 10.0f);

	// Note: The BallProjectileClass and the skeletal mesh/anim blueprints for Mesh1P, FP_Gun, and VR_Gun 
...
{
	// try and fire a BallProjectile
	if (BallProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<ABallProjectile>(BallProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				// spawn the BallProjectile at the muzzle
				World->SpawnActor<ABallProjectile>(BallProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
